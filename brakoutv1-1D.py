...
# SETTING STRATEGI
TIMEFRAME = '1d'        # UBAH KE 1 HARI (DAILY)
...
def send_telegram_alert(symbol, data, image_path):
    ...
    caption = (
        f"🐋 <b>VOLUME + DIVERGENCE ALERT 1D</b>\n"
        f"💎 <b>{symbol}</b>\n"
        f"──────────────────────\n"
        f"📊 <b>Volume Spike:</b> {data['spike_ratio']:.2f}x (Avg 6 Hari)\n"
...
def generate_chart(df, symbol, signal_info):
    try:
        ...
        title_text = f"{symbol} [1D] - Vol {signal_info['spike_ratio']:.1f}x - {signal_info['divergence']}"
...
def analyze_volume_anomaly(df):
    ...
    # Di TF 1D, 6 candle sebelumnya = 6 Hari terakhir (Seminggu)
...
# ==========================================
def main():
    print(f"=== VOLUME + DIVERGENCE HUNTER 1D (v3) ===")
    ...
    while True:
        try:
            print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Scanning 1D (Vol + Div)...")
