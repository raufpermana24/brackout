...
def send_hourly_report():
    ...
    table_str += "<i>*Sinyal: Segitiga, Extreme, Stoch, MA5 Close.</i>"
...
def calculate_all_indicators(df):
    ...
    # --- 1. BBMA (Middle, Top, Low, MA5 High, MA5 Low) ---
    ...
    # --- [BARU] MA 5 CLOSE (Untuk Momentum Tembus Garis) ---
...
def detect_trading_setup(df):
    """
    MENDETEKSI SEMUA SETUP BERDASARKAN HIERARKI (PRIORITAS)
    """
    curr = df.iloc[-1]
    prev = df.iloc[-2]
    ...
    # ==========================================
    # 1. CEK POLA TRIANGLE BREAKOUT
    # ==========================================
    is_triangle, tri_setup_name, tri_dir = detect_triangle_pattern(df)
    if is_triangle:
        return tri_setup_name, tri_dir

    # ==========================================
    # 2. LOGIKA BBMA EXTREME
    # ==========================================
    ...
    # ==========================================
    # 3. LOGIKA STOCHASTIC CROSSOVER (<20 atau >80)
    # ==========================================
    ...
    # ==========================================
    # 4. LOGIKA MA5 CLOSE CROSSOVER [BARU]
    # Ciri: Candle sebelumnya di bawah/atas MA5 Close, 
    # lalu Candle saat ini close memotong melewati MA5 Close tersebut.
    # ==========================================
    ...
    # [BARU] Jika tidak ada Extreme atau Segitiga, pakai MA5 Close Momentum
    elif ma5_cross_up:
        return "MA5 CROSSOVER BUY (Candle Menembus & Close di atas MA5) 📈🟢", "UP"
    elif ma5_cross_down:
        return "MA5 CROSSOVER SELL (Candle Menembus & Close di bawah MA5) 📉🔴", "DOWN"
...
def evaluate_past_prediction(symbol, tf, closed_candle, past_prediction):
    ...
    with stats_lock:
        ...
        # Penjelasan Detail (Khas untuk tiap pola)
        is_triangle_eval = "TRIANGLE" in setup_name
        is_ma5_eval = "MA5 CROSSOVER" in setup_name
        
        if pred_dir == "UP" and "DOWN" in actual_dir:
            if is_triangle_eval:
                eval_text += "👉 False Breakout (Bull Trap): Harga menembus atap segitiga lalu terbanting turun.\n"
            elif is_ma5_eval:
                eval_text += "👉 False MA Breakout: Harga menembus MA5 tapi gagal menahan momentum naik.\n"
            else:
            ...
            if is_triangle_eval:
                eval_text += "👉 False Breakdown (Bear Trap): Harga jatuh menembus lantai tapi bandar memborong balik.\n"
            elif is_ma5_eval:
                eval_text += "👉 False MA Breakdown: Harga menembus MA5 ke bawah tapi gagal menahan momentum turun.\n"
            else:
...
def process_data(symbol, tf):
    try:
        ...
        if last_signaled_candle[symbol][tf] != current_candle['timestamp']:
            ...
            if pred_dir != "NETRAL":
                # Kustomisasi Ikon Judul
                if "TRIANGLE" in setup_name:
                    telegram_message += f"<b>📐 SETUP POLA SEGITIGA DITEMUKAN: {symbol} ({tf})</b>\n"
                elif "MA5" in setup_name:
                    telegram_message += f"<b>⚡ MOMENTUM MA5 CLOSE DITEMUKAN: {symbol} ({tf})</b>\n"
                else:
                ...
                telegram_message += f"• MA5 Close: ${current_candle['MA5_CLOSE']:.2f}\n"
...
# ==========================================
# BACKTEST 1 MINGGU KHUSUS SETUP GABUNGAN
# ==========================================
def run_backtest():
    print("\n⏳ MENGAMBIL DATA 1 MINGGU KE BELAKANG UNTUK BACKTEST...")
    print("Mencari Triangle Breakout, BBMA Extreme, Stoch Cross, & MA5 Close...\n")
    
    send_telegram_message("⏳ <b>Proses Backtest Dimulai...</b>\nMencari semua setup (Triangle, BBMA, Stoch, MA5) BTC/USDT selama 1 minggu terakhir.")
    ...
    telegram_msg = "<b>📊 HASIL BACKTEST SETUP LENGKAP (1 MINGGU) 📊</b>\n"
    telegram_msg += "Filter: Triangle, BBMA, Stoch, MA5 Close\n"
    telegram_msg += "<pre>\n"
    telegram_msg += f"{'TF':<4} | {'TOT':<4} | {'BNR':<4} | {'SLH':<4} | {'AKURASI':<7}\n"
    telegram_msg += "-" * 33 + "\n"
    
    print("\n=======================================================================")
    print("📊 HASIL BACKTEST GABUNGAN KESELURUHAN (1 MINGGU TERAKHIR)")
    print("=======================================================================")
...
def run_bot():
    print("\n======================================================")
    print("🚀 MENJALANKAN BOT ULTIMATE FINAL (Tekan Ctrl+C untuk Stop)")
    print("======================================================\n")

    send_telegram_message("🤖 <b>Bot Ultimate Final Dimulai!</b>\nMendeteksi: Pola Segitiga, BBMA Extreme, Stochastic Crossover murni, & Momentum MA5 Close.")
    global last_report_time
    last_report_time = datetime.now() 
    ...
    try:
        while True:
            print(f"\n======================================================")
            print(f"🔄 Memindai Pola Market Serentak: {datetime.now().strftime('%H:%M:%S')}")
            print(f"======================================================")
...
def main_menu():
    while True:
        ...
        print("======================================================")
        print("🌟 MENU UTAMA BOT CRYPTO ULTIMATE 🌟")
        print("======================================================")
        print("1. 🚀 Jalankan Pendeteksi Pola & Momentum Real-Time")
