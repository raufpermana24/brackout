...
# ------------------------------------------
# [BARU] MEMORI & PENGATURAN STATISTIK 1 JAM
# ------------------------------------------
performance_stats = {tf: {'benar': 0, 'salah': 0} for tf in TIMEFRAMES}
last_report_time = datetime.now() # Menyimpan waktu kapan terakhir kali rekap dikirim
stats_lock = threading.Lock()     # Mengunci data saat dihitung oleh multithreading
print_lock = threading.Lock()     # Mengunci tampilan terminal
...
def send_hourly_report():
    """[BARU] Fungsi untuk mengirim tabel rekapitulasi setiap 1 jam"""
    global last_report_time
    
    # Membuat format Tabel dengan Tag <pre> agar sejajar (Monospace) di Telegram
    ...
    with stats_lock:
        for tf in TIMEFRAMES:
            ...
            # Reset hitungan ke 0 untuk jam berikutnya
    ...
    # Cetak di terminal lokal (hapus tag HTML)
...
def evaluate_past_prediction(symbol, tf, closed_candle, past_prediction):
    """Evaluasi memori lama (Telah Ditambah Perekam Statistik)"""
    ...
    # [BARU] Menambah hitungan jika BENAR
    ...
    # [BARU] Menambah hitungan jika SALAH
    with stats_lock:
...
def run_bot():
    print("======================================================")
    print("🚀 BOT CRYPTO TELEGRAM + BBMA + AUTO REKAP (1 JAM) 🚀")
    print("======================================================\n")

    send_telegram_message("🤖 <b>Bot Prediksi Menyala!</b>\nSiap memantau BTC/USDT.\nTimeframe: 1m, 5m, 15m, 1h, & 4h.\n\n📊 <i>Catatan: Bot akan otomatis mengirimkan Rekap Akurasi setiap 1 Jam.</i>")

    global last_report_time
    last_report_time = datetime.now() # Memulai perhitungan 1 jam

    while True:
        print(f"\n\n======================================================")
        print(f"🔄 Memindai Tren BTC Serentak: {datetime.now().strftime('%H:%M:%S')}")
        print(f"======================================================")
        ...
        # [BARU] CEK APAKAH SUDAH 1 JAM (3600 Detik)
