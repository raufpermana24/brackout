...
def send_telegram_alert(symbol, data, image_path):
    ...
    caption = (
        f"🐋 <b>VOLUME + DIVERGENCE ALERT 6H</b>\n"
        f"💎 <b>{symbol}</b>\n"
        f"──────────────────────\n"
...
def generate_chart(df, symbol, signal_info):
    try:
        ...
        title_text = f"{symbol} [6H] - Vol {signal_info['spike_ratio']:.1f}x - {signal_info['divergence']}"
...
def analyze_volume_anomaly(df):
    ...
    # Di TF 6H, 6 candle sebelumnya = 36 Jam terakhir
...
# ==========================================
def main():
    print(f"=== VOLUME + DIVERGENCE HUNTER 6H (v3) ===")
    ...
    while True:
        try:
            print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Scanning 6H (Vol + Div)...")
