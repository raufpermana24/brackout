...
# SETTING STRATEGI
TIMEFRAME = '1w'        # UBAH KE 1 MINGGU (WEEKLY)
VOL_MULTIPLIER = 2.0    # Volume harus 2x dari rata-rata
LIMIT = 100             # Ambil 100 candle (100 Minggu)
...
def send_telegram_alert(symbol, data, image_path):
    ...
    caption = (
        f"🐋 <b>VOLUME + DIVERGENCE ALERT 1W</b>\n"
        f"💎 <b>{symbol}</b>\n"
        f"──────────────────────\n"
        f"📊 <b>Volume Spike:</b> {data['spike_ratio']:.2f}x (Avg 6 Minggu)\n"
...
def generate_chart(df, symbol, signal_info):
    try:
        ...
        title_text = f"{symbol} [1W] - Vol {signal_info['spike_ratio']:.1f}x - {signal_info['divergence']}"
...
def analyze_volume_anomaly(df):
    ...
    # Di TF 1W, 6 candle sebelumnya = 6 Minggu terakhir
...
def analyze_bbma_setup(df):
    ...
    if signal_data:
        ...
        signal_data['divergence'] = divergence_status
        return signal_data
...
# ==========================================
def main():
    print(f"=== VOLUME + DIVERGENCE HUNTER 1W (v3) ===")
    ...
    while True:
        try:
            print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Scanning 1W (Vol + Div)...")
