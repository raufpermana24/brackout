...
# ==========================================
# MANAJEMEN DATA VPS & VOLUME TIKER
# ==========================================
def update_volumes():
    """Mengambil data Volume USDT 24 Jam secara keseluruhan"""
    global coin_volumes
    try:
        tickers = exchange.fetch_tickers()
        for sym in SYMBOLS:
            if sym in tickers:
                val = tickers[sym].get('quoteVolume', 0)
                if val is not None:
                    coin_volumes[sym] = float(val)
    except Exception as e:
        with print_lock: print(f"⚠️ Gagal update volume 24J: {e}")
...
def generate_candlestick_chart(symbol, tf, df):
    ...
    y = df_chart['close'].values
    slope, intercept = np.polyfit(x, y, 1)
    trendline_y = slope * x + intercept
    
    pct_slope = (slope / y[-1]) * 100
    if pct_slope > 0.03: trend_color = '#00ff88'
    elif pct_slope < -0.03: trend_color = '#ff3366'
    else: trend_color = '#aaaaaa'

    ax.plot(x, trendline_y, color=trend_color, linestyle='--', linewidth=2, alpha=0.8, label='Auto Trendline')

    clean_sym = symbol.replace(':USDT', '')
    ax.set_title(f"Grafik {clean_sym} ({tf}) - Algorithmic Analysis", color='white', fontsize=12)
...
def get_summary_message():
    ...
    msg += "\n<i>👉 Ketik timeframe yang ingin dianalisis (contoh: <b>/5m</b>, <b>/1h</b>, <b>/4h</b>, <b>/1d</b>)</i>\n"
    msg += "<i>🔥 Ketik <b>/hot</b> untuk melihat koin dengan volume tertinggi hari ini.</i>"
    return msg

def send_top_volume_coins(chat_id):
    if not coin_volumes:
        send_telegram_message("⏳ Data volume belum siap. Silakan tunggu beberapa saat.", chat_id)
        return
        
    sorted_coins = sorted(coin_volumes.items(), key=lambda x: x[1], reverse=True)[:15]
    
    msg = "<b>🔥 TOP 15 KOIN PALING HOT (VOLUME 24J)</b>\n"
    msg += "<i>Koin dengan likuiditas tinggi sangat cocok & aman untuk trading.</i>\n\n"
    msg += "<pre>\n"
    msg += f"{'KOIN':<8} | {'VOLUME (USDT)':<15}\n"
    msg += "-" * 26 + "\n"
    
    for sym, vol in sorted_coins:
        clean_sym = sym.split('/')[0]
        if vol >= 1_000_000_000: vol_str = f"${vol/1_000_000_000:.2f} Milyar"
        else: vol_str = f"${vol/1_000_000:.2f} Juta"
        msg += f"{clean_sym:<8} | {vol_str}\n"
        
    msg += "</pre>\n👉 <i>Gunakan timeframe /15m atau /1h untuk mencari setup di koin-koin ini.</i>"
...
def get_open_interest_analysis(symbol, tf, current_price, prev_price):
    try:
        ...
        url = f"https://fapi.binance.com/futures/data/openInterestHist?symbol={clean_sym}&period={api_tf}&limit=2"
        ...
        if len(res) >= 2:
            ...
            if oi_up and price_up: return "🟢 Bullish Menguat (OI Naik & Harga Naik)", 2
            elif oi_up and not price_up: return "🔴 Bearish Menguat (OI Naik & Harga Turun)", -2
            elif not oi_up and price_up: return "⚠️ Bullish Melemah (Short Covering/Fakeout)", -1
            elif not oi_up and not price_up: return "⚠️ Bearish Melemah (Long Liquidation)", 1
    except: pass
...
def send_detailed_setups(chat_id, tf_request):
    ...
    send_telegram_message(f"⏳ <b>Memproses {len(setups)} sinyal mentah...</b>\nMenerapkan Smart Filter & Algoritma Trendline. Memakan waktu beberapa detik...", chat_id)
    ...
    for sym in sorted_syms:
        ...
        vol = coin_volumes.get(sym, 0)
        if vol < 10_000_000: continue 

        if vol >= 1_000_000_000: vol_str = f"${vol/1_000_000_000:.2f} Milyar"
        elif vol >= 1_000_000: vol_str = f"${vol/1_000_000:.2f} Juta"
        ...
        try:
            ...
            else: whale_stat, whale_score = f"⚪ Netral", 0
        except:
            whale_stat, whale_score = "⚪ Data Paus Tidak Tersedia", 0

        oi_tf_check = tf_request
        if tf_request == '1d': oi_tf_check = '1d'
        oi_stat, oi_score = get_open_interest_analysis(sym, oi_tf_check, data['price'], data['prev_price'])
        ...
        if dir_long:
            if "DOWNTREND" not in trend_status:
                if is_super_setup and oi_score >= 0: is_important = True 
                elif oi_score == 2: is_important = True 
                elif whale_score == 1 and ("CROSS" in setup_name or "STOCH" in setup_name) and oi_score >= 0: is_important = True 
        else:
        ...
        if is_important:
            ...
            tf_label = tf_request
            if tf_request == '1d': tf_label = '1 Hari (Makro/Swing)'
            elif tf_request == '4h': tf_label = '4 Jam (Swing)'
            
            caption = f"<b>🤖 ALGO VIP: {clean_sym}/USDT | {dir_icon}</b>\n"
            caption += f"⏰ <b>Timeframe:</b> {tf_label}\n"
            caption += f"📝 <b>Pola Setup:</b> {setup_name}\n"
            caption += f"🛤️ <b>Trendline (50 Candle):</b> {trend_status}\n"
            caption += f"💰 <b>Volume 24J:</b> {vol_str}\n"
    ...
    if len(vip_signals_to_send) == 0:
        send_telegram_message(f"🗑️ <b>Filter Selesai!</b>\nDari {len(setups)} sinyal awal di {tf_request}, <b>TIDAK ADA</b> yang lolos standar VIP.\n<i>(Sinyal dibuang karena melawan Trend Utama, Volume Rendah, atau Rawan Fakeout).</i>", chat_id)
        return

    send_telegram_message(f"🎯 <b>Filter Selesai!</b>\nDitemukan <b>{len(vip_signals_to_send)} Sinyal HIGH PROBABILITY</b>. Mengirim grafik...", chat_id)
            ...
            if 'result' in response:
                for update in response['result']:
                    ...
                    if msg_data and 'text' in msg_data:
                        ...
                        if 'hot' in text or 'vol' in text:
                            send_top_volume_coins(chat_id)
                            continue
...
# ==========================================
def analyze_trend(df, window=50):
    if len(df) < window: return "SIDEWAYS ⚪", 0
    df_trend = df.tail(window)
    x = np.arange(len(df_trend))
    y = df_trend['close'].values
    slope, _ = np.polyfit(x, y, 1)
    pct_slope = (slope / y[-1]) * 100 
    if pct_slope > 0.03: return "UPTREND 📈", pct_slope
    elif pct_slope < -0.03: return "DOWNTREND 📉", pct_slope
    else: return "SIDEWAYS ⚪", pct_slope
...
def detect_trading_setup(df):
    ...
    elif bbma_sell: return "EXTREME SELL (MA5 Keluar Top BB)", "DOWN"
    elif bbma_buy: return "EXTREME BUY (MA5 Keluar Low BB)", "UP"
    elif stoch_sell: return "STOCH SELL (Garis %K Potong ke Bawah)", "DOWN"
    elif stoch_buy: return "STOCH BUY (Garis %K Potong ke Atas)", "UP"
    elif ma813_cross_up: return "MA8/MA13 GOLDEN CROSS", "UP"
    elif ma813_cross_down: return "MA8/MA13 DEATH CROSS", "DOWN"
...
def process_data(symbol, tf):
    try:
        ...
        trend_status, trend_slope = analyze_trend(df)
...
def run_bot():
    is_restored = initialize_memory()
    print("\n======================================================")
    print("🚀 MENJALANKAN BOT (VERSI VPS DAEMON)")
    print("======================================================\n")
    ...
    try:
        while True:
            ...
            with print_lock:
                print(f"\n🔄 [AUTO-SCAN] Memulai siklus pemindaian {len(SYMBOLS)} koin...")
                
            threading.Thread(target=update_volumes).start()
...
def main_menu():
    while True:
        ...
        print("======================================================")
        print("🌟 MENU UTAMA BOT VPS (ULTIMATE LIVE VERSION) 🌟")
        print("======================================================")
        print("1. 🚀 Jalankan Auto-Scan (Live Trading & Interaktif)")
